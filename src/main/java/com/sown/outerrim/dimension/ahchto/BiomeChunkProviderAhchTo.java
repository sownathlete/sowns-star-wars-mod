...
public class BiomeChunkProviderAhchTo
extends ChunkProviderGenerate {
    ...
    public BiomeChunkProviderAhchTo(World world, long seed, boolean features) {
    ...
    public void populate(IChunkProvider par1IChunkProvider, int par2, int par3) {
        ...
        removeUnwantedFluids(par2, par3);
    ...
    private void removeUnwantedFluids(int chunkX, int chunkZ) {
        int worldX = chunkX * 16;
        int worldZ = chunkZ * 16;
        Block liquidAcid = FluidRegister.getRegisteredFluidBlock("liquid_acid");

        for (int x = 0; x < 16; ++x) {
            for (int z = 0; z < 16; ++z) {
                for (int y = 0; y < 256; ++y) { // Full height scan
                    int worldXPos = worldX + x;
                    int worldZPos = worldZ + z;

                    Block block = this.worldObj.getBlock(worldXPos, y, worldZPos);

                    // Remove lava and acid anywhere
                    if (block == Blocks.lava || block == Blocks.flowing_lava || block == liquidAcid) {
                        this.worldObj.setBlock(worldXPos, y, worldZPos, Blocks.air);
                    }

                    // Remove water **only above sea level (y > 63)**
                    if (y > 63 && (block == Blocks.water || block == Blocks.flowing_water)) {
                        this.worldObj.setBlock(worldXPos, y, worldZPos, Blocks.air);
                    }
                }
            }
    ...
    public void replaceBlocksForBiome(int par1, int par2, Block[] par3ArrayOfBlock, byte[] par4ArrayOfByte, 
        ...
        ChunkProviderEvent.ReplaceBiomeBlocks event = new ChunkProviderEvent.ReplaceBiomeBlocks(
                this, par1, par2, par3ArrayOfBlock, par4ArrayOfByte, par5ArrayOfBiomeGenBase, this.worldObj);
        ...
        double d0 = 0.03125;
        this.stoneNoise = this.noiseGen4.func_151599_a(this.stoneNoise, par1 * 16, par2 * 16, 
        ...
        // Initialize random with chunk coordinates for consistent results
        Random rand = new Random(par1 * 341873128712L + par2 * 132897987541L);

        // Get the registered acid block from FluidRegistry
        Block liquidAcid = FluidRegister.getRegisteredFluidBlock("liquid_acid");

        // Loop through the entire chunk to modify blocks
        for (int k = 0; k < 16; ++k) {
            for (int l = 0; l < 16; ++l) {
                for (int y = 0; y < 256; ++y) { // Loop through height levels
                    int index = (k * 16 + l) * 256 + y; // Calculate block index
                    Block currentBlock = par3ArrayOfBlock[index];

                    // Replace lava and acid anywhere in the chunk with air
                    if (currentBlock == Blocks.lava || currentBlock == liquidAcid) {
                        par3ArrayOfBlock[index] = Blocks.air;
                    }

                    // Replace water with air only above sea level (y > 63)
                    if (y > 63 && (currentBlock == Blocks.water || currentBlock == Blocks.flowing_water)) {
                        par3ArrayOfBlock[index] = Blocks.air;
                    }

                    // Modify grass block behavior
                    if (currentBlock == Blocks.grass) { 
                        if (y <= 61) {
                            // Convert grass blocks at or below y=61 to gravel
                            par3ArrayOfBlock[index] = Blocks.gravel;
                        } else if (rand.nextInt(5) == 0) { // 1 in 5 chance (20%) to replace with cobblestone
                            par3ArrayOfBlock[index] = Blocks.cobblestone;
                        }
                    }
                }
            }
