...
public class WorldChunkManagerAhchTo
extends DimensionHelperChunkMgr {
    ...
    protected WorldChunkManagerAhchTo() {
        this.biomesToSpawn.add(AhchToProvider.ahchTo);
    }

    public WorldChunkManagerAhchTo(long seed) {
        this();
        GenLayer[] agenlayer = GenLayerBiomeAhchTo.makeTheWorld(seed);
        this.unzoomedBiomes = agenlayer[0];
        this.zoomedBiomes = agenlayer[1];
    }

    public WorldChunkManagerAhchTo(World world) {
        this(world.getSeed());
    }

    @Override
    public BiomeGenBase getBiome() {
        return AhchToProvider.ahchTo;
    }
    ...
    public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int z, int length, int width) {
        ...
        for (int i = 0; i < length * width; ++i) {
            par1ArrayOfBiomeGenBase[i] = arrayOfInts[i] >= 0 ? BiomeGenBase.getBiome(arrayOfInts[i]) : AhchToProvider.ahchTo;
        }
    ...
    public BiomeGenBase[] getBiomeGenAt(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int y, int width, int length, boolean cacheFlag) {
        ...
        for (int i = 0; i < width * length; ++i) {
            par1ArrayOfBiomeGenBase[i] = ai[i] >= 0 ? BiomeGenBase.getBiome(ai[i]) : AhchToProvider.ahchTo;
        }
        return par1ArrayOfBiomeGenBase;
    }
