...
public class WorldChunkManagerAjanKloss
extends DimensionHelperChunkMgr {
    ...
    protected WorldChunkManagerAjanKloss() {
        this.biomesToSpawn.add(AjanKlossProvider.ajanKloss);
    }

    public WorldChunkManagerAjanKloss(long seed) {
        this();
        GenLayer[] agenlayer = GenLayerBiomeAjanKloss.makeTheWorld(seed);
        this.unzoomedBiomes = agenlayer[0];
        this.zoomedBiomes = agenlayer[1];
    }

    public WorldChunkManagerAjanKloss(World world) {
        this(world.getSeed());
    }

    @Override
    public BiomeGenBase getBiome() {
        return AjanKlossProvider.ajanKloss;
    }
    ...
    public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int z, int length, int width) {
        ...
        for (int i = 0; i < length * width; ++i) {
            par1ArrayOfBiomeGenBase[i] = arrayOfInts[i] >= 0 ? BiomeGenBase.getBiome(arrayOfInts[i]) : AjanKlossProvider.ajanKloss;
        }
    ...
    public BiomeGenBase[] getBiomeGenAt(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int y, int width, int length, boolean cacheFlag) {
        ...
        for (int i = 0; i < width * length; ++i) {
            par1ArrayOfBiomeGenBase[i] = ai[i] >= 0 ? BiomeGenBase.getBiome(ai[i]) : AjanKlossProvider.ajanKloss;
        }
        return par1ArrayOfBiomeGenBase;
    }
