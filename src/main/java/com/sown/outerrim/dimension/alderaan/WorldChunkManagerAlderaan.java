...
public class WorldChunkManagerAlderaan
extends DimensionHelperChunkMgr {
    ...
    protected WorldChunkManagerAlderaan() {
        this.biomesToSpawn.add(BiomeGenBase.forestHills);
    }

    public WorldChunkManagerAlderaan(long seed) {
        this();
        GenLayer[] agenlayer = GenLayerBiomeAlderaan.makeTheWorld(seed);
        this.unzoomedBiomes = agenlayer[0];
        this.zoomedBiomes = agenlayer[1];
    }

    public WorldChunkManagerAlderaan(World world) {
        this(world.getSeed());
    }
    ...
    public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int z, int length, int width) {
        ...
        for (int i = 0; i < length * width; ++i) {
            par1ArrayOfBiomeGenBase[i] = arrayOfInts[i] >= 0 ? BiomeGenBase.getBiome(arrayOfInts[i]) : BiomeGenBase.forestHills;
        }
    ...
    public BiomeGenBase[] getBiomeGenAt(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int y, int width, int length, boolean cacheFlag) {
        ...
        for (int i = 0; i < width * length; ++i) {
            par1ArrayOfBiomeGenBase[i] = ai[i] >= 0 ? BiomeGenBase.getBiome(ai[i]) : BiomeGenBase.forestHills;
        }
        return par1ArrayOfBiomeGenBase;
    }
