...
public class BiomeChunkProviderBespin
extends ChunkProviderGenerate {
    ...
    public BiomeChunkProviderBespin(World world, long seed, boolean features) {
    ...
    public Chunk provideChunk(int par1, int par2) {
        ...
        Block[] ablock = new Block[65536];  // Initialize to air blocks by default
        byte[] abyte = new byte[65536];

        // Generate only air blocks, effectively removing bedrock
        for (int i = 0; i < 16; ++i) {
            for (int j = 0; j < 16; ++j) {
                for (int h = 0; h < 256; ++h) {
                    int index = (i * 16 + j) * 256 + h;
                    ablock[index] = Blocks.air;
                }
            }
        }
        ...
        for (int k = 0; k < abyte1.length; ++k) {
            abyte1[k] = (byte)OuterRimResources.ConfigOptions.biomeBespinId;  // Use the biome you've defined for Bespin
        }
    ...
    @Override
    public void replaceBlocksForBiome(int par1, int par2, Block[] blocks, byte[] bytes, BiomeGenBase[] biomes) {
        // Do nothing. We've already set up bedrock in the provideChunk method.
    }
