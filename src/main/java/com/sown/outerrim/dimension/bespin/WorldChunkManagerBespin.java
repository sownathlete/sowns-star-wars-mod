...
public class WorldChunkManagerBespin
extends DimensionHelperChunkMgr {
    ...
    protected WorldChunkManagerBespin() {
        this.biomesToSpawn.add(BespinProvider.bespin);
    }

    public WorldChunkManagerBespin(long seed) {
        this();
        GenLayer[] agenlayer = GenLayerBiomeBespin.makeTheWorld(seed);
        this.unzoomedBiomes = agenlayer[0];
        this.zoomedBiomes = agenlayer[1];
    }

    public WorldChunkManagerBespin(World world) {
        this(world.getSeed());
    }

    @Override
    public BiomeGenBase getBiome() {
        return BespinProvider.bespin;
    }
    ...
    public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int z, int length, int width) {
        ...
        for (int i = 0; i < length * width; ++i) {
            par1ArrayOfBiomeGenBase[i] = arrayOfInts[i] >= 0 ? BiomeGenBase.getBiome(arrayOfInts[i]) : BespinProvider.bespin;
        }
    ...
    public BiomeGenBase[] getBiomeGenAt(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int y, int width, int length, boolean cacheFlag) {
        ...
        for (int i = 0; i < width * length; ++i) {
            par1ArrayOfBiomeGenBase[i] = ai[i] >= 0 ? BiomeGenBase.getBiome(ai[i]) : BespinProvider.bespin;
        }
        return par1ArrayOfBiomeGenBase;
    }
