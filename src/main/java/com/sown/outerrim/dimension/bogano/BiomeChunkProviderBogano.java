...
public class BiomeChunkProviderBogano
extends ChunkProviderGenerate {
    ...
    public BiomeChunkProviderBogano(World world, long seed, boolean features) {
    ...
    public void replaceBlocksForBiome(int chunkX, int chunkZ, Block[] blocks, byte[] metadata, BiomeGenBase[] biomes) {
        ChunkProviderEvent.ReplaceBiomeBlocks event = new ChunkProviderEvent.ReplaceBiomeBlocks(this, chunkX, chunkZ, blocks, metadata, biomes, this.worldObj);
        MinecraftForge.EVENT_BUS.post(event);
        if (event.getResult() == Event.Result.DENY) return;

        double d0 = 0.03125;
        this.stoneNoise = this.noiseGen4.func_151599_a(this.stoneNoise, chunkX * 16, chunkZ * 16, 16, 16, d0 * 2.0, d0 * 2.0, 1.0);

        for (int k = 0; k < 16; ++k) {
            for (int l = 0; l < 16; ++l) {
                BiomeGenBase biome = biomes[l + k * 16];
                biome.genTerrainBlocks(this.worldObj, this.rand, blocks, metadata, chunkX * 16 + k, chunkZ * 16 + l, this.stoneNoise[l + k * 16]);
            }
        }
    }
    ...
    public Chunk provideChunk(int chunkX, int chunkZ) {
        ...
        Block boganoRockLight = BlockRegister.getRegisteredBlock("boganoRockLight");
        Block karniteOre = BlockRegister.getRegisteredBlock("karniteOre");
        Block boganoGrass = BlockRegister.getRegisteredBlock("boganoGrass");

        // Replace all stone with boganoRockLight
        for (int i = 0; i < blocks.length; i++) {
            if (blocks[i] == Blocks.stone) {
                blocks[i] = boganoRockLight;
            }
        }

        // Remove stacked boganoGrass by replacing anything under the topmost one with light rock
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                boolean foundTopGrass = false;
                for (int y = 255; y >= 0; y--) {
                    int index = (z * 16 + x) * 256 + y;
                    if (blocks[index] == boganoGrass) {
                        if (!foundTopGrass) {
                            foundTopGrass = true;
                        } else {
                            blocks[index] = boganoRockLight;
                        }
                    }
                }
            }
     ...
     // Sprinkle in rare karniteOre clusters (unchanged)
        for (int attempt = 0; attempt < 5; attempt++) {
            if (rand.nextInt(6) != 0) continue;

            int localX = rand.nextInt(16);
            int localY = rand.nextInt(27) + 4;
                    ...
                    for (int dz = -1; dz <= 1; dz++) {
                        if (rand.nextInt(6) == 0) {
                            ...
                            if (x >= 0 && x < 16 && y >= 0 && y < 256 && z >= 0 && z < 16) {
                                int index = (z * 16 + x) * 256 + y;
                                if (blocks[index] == boganoRockLight) {
                                    blocks[index] = karniteOre;
                                }
                            }
                        }
                    }
        ...
        // Vanilla ore generation (reduced rates except coal)
        for (int i = 0; i < 3; i++) { // Coal, more common
            int x = rand.nextInt(16);
            int y = rand.nextInt(64); // normal range for coal
            int z = rand.nextInt(16);
            int index = (z * 16 + x) * 256 + y;
            if (blocks[index] == boganoRockLight) {
                blocks[index] = Blocks.coal_ore;
            }
        }

        if (rand.nextInt(3) == 0) { // Iron
            int x = rand.nextInt(16);
            int y = rand.nextInt(54);
            int z = rand.nextInt(16);
            int index = (z * 16 + x) * 256 + y;
            if (blocks[index] == boganoRockLight) {
                blocks[index] = Blocks.iron_ore;
            }
        }

        if (rand.nextInt(4) == 0) { // Gold
            int x = rand.nextInt(16);
            int y = rand.nextInt(32);
            int z = rand.nextInt(16);
            int index = (z * 16 + x) * 256 + y;
            if (blocks[index] == boganoRockLight) {
                blocks[index] = Blocks.gold_ore;
            }
        }

        if (rand.nextInt(5) == 0) { // Redstone
            int x = rand.nextInt(16);
            int y = rand.nextInt(16);
            int z = rand.nextInt(16);
            int index = (z * 16 + x) * 256 + y;
            if (blocks[index] == boganoRockLight) {
                blocks[index] = Blocks.redstone_ore;
            }
        }

        if (rand.nextInt(6) == 0) { // Lapis
            int x = rand.nextInt(16);
            int y = rand.nextInt(24);
            int z = rand.nextInt(16);
            int index = (z * 16 + x) * 256 + y;
            if (blocks[index] == boganoRockLight) {
                blocks[index] = Blocks.lapis_ore;
            }
        }

        if (rand.nextInt(6) == 0) { // Diamond
            int x = rand.nextInt(16);
            int y = rand.nextInt(16);
            int z = rand.nextInt(16);
            int index = (z * 16 + x) * 256 + y;
            if (blocks[index] == boganoRockLight) {
                blocks[index] = Blocks.diamond_ore;
            }
        }

        if (rand.nextInt(8) == 0) { // Emerald
            int x = rand.nextInt(16);
            int y = rand.nextInt(28);
            int z = rand.nextInt(16);
            int index = (z * 16 + x) * 256 + y;
            if (blocks[index] == boganoRockLight) {
                blocks[index] = Blocks.emerald_ore;
            }
        }
    ...
    private boolean isExposedToSky(Block[] blocks, int x, int y, int z) {
        for (int dy = y + 1; dy < 256; dy++) {
            int index = (z * 16 + x) * 256 + dy;
