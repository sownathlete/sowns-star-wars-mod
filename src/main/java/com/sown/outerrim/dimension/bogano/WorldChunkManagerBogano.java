...
public class WorldChunkManagerBogano
extends DimensionHelperChunkMgr {
    ...
    protected WorldChunkManagerBogano() {
        this.biomesToSpawn.add(BoganoProvider.BoganoPlains);
    }

    public WorldChunkManagerBogano(long seed) {
        this();
        GenLayer[] agenlayer = GenLayerBiomeBogano.makeTheWorld(seed);
        this.unzoomedBiomes = agenlayer[0];
        this.zoomedBiomes = agenlayer[1];
    }

    public WorldChunkManagerBogano(World world) {
        this(world.getSeed());
    }

    @Override
    public BiomeGenBase getBiome() {
        return BoganoProvider.BoganoPlains;
    }
    ...
    public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int z, int length, int width) {
        ...
        for (int i = 0; i < length * width; ++i) {
            par1ArrayOfBiomeGenBase[i] = arrayOfInts[i] >= 0 ? BiomeGenBase.getBiome(arrayOfInts[i]) : BoganoProvider.BoganoPlains;
        }
    ...
    public BiomeGenBase[] getBiomeGenAt(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int y, int width, int length, boolean cacheFlag) {
        ...
        for (int i = 0; i < width * length; ++i) {
            par1ArrayOfBiomeGenBase[i] = ai[i] >= 0 ? BiomeGenBase.getBiome(ai[i]) : BoganoProvider.BoganoPlains;
        }
        return par1ArrayOfBiomeGenBase;
    }
