...
public class WorldChunkManagerBothawui extends DimensionHelperChunkMgr {
	private GenLayer unzoomedBiomes;
	private GenLayer zoomedBiomes;
	private BiomeCache biomeCache = new BiomeCache(this);
	private List<BiomeGenBase> biomesToSpawn = new ArrayList<>();

	protected WorldChunkManagerBothawui() {
		biomesToSpawn.add(BiomeGenBase.forest);
		biomesToSpawn.add(BiomeGenBase.plains);
		biomesToSpawn.add(BiomeGenBase.savanna);
		biomesToSpawn.add(BiomeGenBase.extremeHills);
		biomesToSpawn.add(BiomeGenBase.taiga);
		biomesToSpawn.add(BiomeGenBase.icePlains);
	}

	public WorldChunkManagerBothawui(long seed) {
		this();
		GenLayer[] agenlayer = GenLayerBiomeBothawui.makeTheWorld(seed);
		unzoomedBiomes = agenlayer[0];
		zoomedBiomes = agenlayer[1];
	}

	public WorldChunkManagerBothawui(World world) {
		this(world.getSeed());
	}
	...
	public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] par1Array, int x, int z, int length, int width) {
		...
		for (int i = 0; i < length * width; ++i) {
			...
			if (par1Array[i] == null)
				par1Array[i] = BiomeGenBase.plains;
		}
		return par1Array;
	}
