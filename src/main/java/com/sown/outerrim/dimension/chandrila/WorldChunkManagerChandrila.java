...
public class WorldChunkManagerChandrila extends DimensionHelperChunkMgr {
	private GenLayer unzoomedBiomes;
	private GenLayer zoomedBiomes;
	private BiomeCache biomeCache = new BiomeCache(this);
	private List<BiomeGenBase> biomesToSpawn = new ArrayList<>();

	protected WorldChunkManagerChandrila() {
		biomesToSpawn.add(BiomeGenBase.plains);
		biomesToSpawn.add(BiomeGenBase.forest);
		biomesToSpawn.add(BiomeGenBase.forestHills);
		biomesToSpawn.add(BiomeGenBase.beach);
		biomesToSpawn.add(BiomeGenBase.extremeHillsEdge);
	}

	public WorldChunkManagerChandrila(long seed) {
		this();
		GenLayer[] agenlayer = GenLayerBiomeChandrila.makeTheWorld(seed);
		unzoomedBiomes = agenlayer[0];
		zoomedBiomes = agenlayer[1];
	}

	public WorldChunkManagerChandrila(World world) {
		this(world.getSeed());
	}
