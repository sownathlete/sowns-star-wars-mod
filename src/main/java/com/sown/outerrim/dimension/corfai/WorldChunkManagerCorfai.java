...
public class WorldChunkManagerCorfai extends DimensionHelperChunkMgr {
	...
	protected WorldChunkManagerCorfai() {
		this.biomesToSpawn.add(BiomeGenBase.forest);
		this.biomesToSpawn.add(BiomeGenBase.plains);
	}

	public WorldChunkManagerCorfai(long seed) {
		this();
		GenLayer[] agenlayer = GenLayerBiomeCorfai.makeTheWorld(seed);
		this.unzoomedBiomes = agenlayer[0];
		this.zoomedBiomes = agenlayer[1];
	}

	public WorldChunkManagerCorfai(World world) {
		this(world.getSeed());
	}
	...
	public BiomeGenBase getBiomeGenAt(int x, int z) {
		...
		return biome != null ? biome : BiomeGenBase.forest;
	}
	...
	public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] arr, int x, int z, int w, int h) {
		...
		for (int i = 0; i < w * h; ++i)
			arr[i] = ids[i] >= 0 ? BiomeGenBase.getBiome(ids[i]) : BiomeGenBase.forest;
		return arr;
	}
