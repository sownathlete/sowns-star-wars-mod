...
public class GenLayerBiomeCrait extends GenLayer {

    public GenLayerBiomeCrait(long seed) {
        super(seed);
    }
        ...
        for (int y = 0; y < areaHeight; y++) {
            for (int x = 0; x < areaWidth; x++) {
                ...
                if (nextInt < 30) {  // 30% chance
                    biomes[x + y * areaWidth] = OuterRimResources.ConfigOptions.biomeCraitId;
                } else if (nextInt < 65) {  // 35% chance
                    biomes[x + y * areaWidth] = OuterRimResources.ConfigOptions.biomeCraitId;
                } else if (nextInt < 88) {  // 23% chance
                    biomes[x + y * areaWidth] = OuterRimResources.ConfigOptions.biomeCraitMountainsId;
    ...
    public static GenLayer[] makeTheWorld(long seed) {
        GenLayer biomes = new GenLayerCraitBiomes(1L);
