...
public class WorldChunkManagerCrait
extends DimensionHelperChunkMgr {
    ...
    protected WorldChunkManagerCrait() {
        this.biomesToSpawn.add(CraitProvider.crait);
    }

    public WorldChunkManagerCrait(long seed) {
        this();
        GenLayer[] agenlayer = GenLayerBiomeCrait.makeTheWorld(seed);
        this.unzoomedBiomes = agenlayer[0];
        this.zoomedBiomes = agenlayer[1];
    }

    public WorldChunkManagerCrait(World world) {
        this(world.getSeed());
    }

    @Override
    public BiomeGenBase getBiome() {
        return CraitProvider.crait;
    }
    ...
    public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int z, int length, int width) {
        ...
        for (int i = 0; i < length * width; ++i) {
            par1ArrayOfBiomeGenBase[i] = arrayOfInts[i] >= 0 ? BiomeGenBase.getBiome(arrayOfInts[i]) : CraitProvider.crait;
        }
    ...
    public BiomeGenBase[] getBiomeGenAt(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int y, int width, int length, boolean cacheFlag) {
        ...
        for (int i = 0; i < width * length; ++i) {
            par1ArrayOfBiomeGenBase[i] = ai[i] >= 0 ? BiomeGenBase.getBiome(ai[i]) : CraitProvider.crait;
        }
        return par1ArrayOfBiomeGenBase;
    }
