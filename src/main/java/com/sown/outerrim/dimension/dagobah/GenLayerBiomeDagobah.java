...
public class GenLayerBiomeDagobah extends GenLayer {

    public GenLayerBiomeDagobah(long seed) {
        super(seed);
    }
        ...
        for (int y = 0; y < areaHeight; y++) {
            for (int x = 0; x < areaWidth; x++) {
                ...
                if (nextInt < 30) {  // 30% chance
                    biomes[x + y * areaWidth] = OuterRimResources.ConfigOptions.biomeDagobahId;
    ...
    public static GenLayer[] makeTheWorld(long seed) {
        GenLayer biomes = new GenLayerDagobahBiomes(1L);
