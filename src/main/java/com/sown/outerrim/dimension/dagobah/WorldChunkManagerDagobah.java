...
public class WorldChunkManagerDagobah
extends DimensionHelperChunkMgr {
    ...
    protected WorldChunkManagerDagobah() {
        this.biomesToSpawn.add(DagobahProvider.dagobah);
    }

    public WorldChunkManagerDagobah(long seed) {
        this();
        GenLayer[] agenlayer = GenLayerBiomeDagobah.makeTheWorld(seed);
        this.unzoomedBiomes = agenlayer[0];
        this.zoomedBiomes = agenlayer[1];
    }

    public WorldChunkManagerDagobah(World world) {
        this(world.getSeed());
    }

    @Override
    public BiomeGenBase getBiome() {
        return DagobahProvider.dagobah;
    }
    ...
    public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int z, int length, int width) {
        ...
        for (int i = 0; i < length * width; ++i) {
            par1ArrayOfBiomeGenBase[i] = arrayOfInts[i] >= 0 ? BiomeGenBase.getBiome(arrayOfInts[i]) : DagobahProvider.dagobah;
        }
    ...
    public BiomeGenBase[] getBiomeGenAt(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int y, int width, int length, boolean cacheFlag) {
        ...
        for (int i = 0; i < width * length; ++i) {
            par1ArrayOfBiomeGenBase[i] = ai[i] >= 0 ? BiomeGenBase.getBiome(ai[i]) : DagobahProvider.dagobah;
        }
        return par1ArrayOfBiomeGenBase;
    }
