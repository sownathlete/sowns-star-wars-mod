...
public class WorldChunkManagerDantooine
extends DimensionHelperChunkMgr {
    ...
    protected WorldChunkManagerDantooine() {
        this.biomesToSpawn.add(BiomeGenBase.savanna);
    }

    public WorldChunkManagerDantooine(long seed) {
        this();
        GenLayer[] agenlayer = GenLayerBiomeDantooine.makeTheWorld(seed);
        this.unzoomedBiomes = agenlayer[0];
        this.zoomedBiomes = agenlayer[1];
    }

    public WorldChunkManagerDantooine(World world) {
        this(world.getSeed());
    }
    ...
    public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int z, int length, int width) {
        ...
        for (int i = 0; i < length * width; ++i) {
            par1ArrayOfBiomeGenBase[i] = arrayOfInts[i] >= 0 ? BiomeGenBase.getBiome(arrayOfInts[i]) : BiomeGenBase.savanna;
        }
    ...
    public BiomeGenBase[] getBiomeGenAt(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int y, int width, int length, boolean cacheFlag) {
        ...
        for (int i = 0; i < width * length; ++i) {
            par1ArrayOfBiomeGenBase[i] = ai[i] >= 0 ? BiomeGenBase.getBiome(ai[i]) : BiomeGenBase.savanna;
        }
        return par1ArrayOfBiomeGenBase;
    }
