...
public class WorldChunkManagerEndor
extends DimensionHelperChunkMgr {
    ...
    protected WorldChunkManagerEndor() {
        this.biomesToSpawn.add(BiomeGenBase.taiga);
    }

    public WorldChunkManagerEndor(long seed) {
        this();
        GenLayer[] agenlayer = GenLayerBiomeEndor.makeTheWorld(seed);
        this.unzoomedBiomes = agenlayer[0];
        this.zoomedBiomes = agenlayer[1];
    }

    public WorldChunkManagerEndor(World world) {
        this(world.getSeed());
    }
    ...
    public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int z, int length, int width) {
        ...
        for (int i = 0; i < length * width; ++i) {
            par1ArrayOfBiomeGenBase[i] = arrayOfInts[i] >= 0 ? BiomeGenBase.getBiome(arrayOfInts[i]) : BiomeGenBase.taiga;
        }
    ...
    public BiomeGenBase[] getBiomeGenAt(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int y, int width, int length, boolean cacheFlag) {
        ...
        for (int i = 0; i < width * length; ++i) {
            par1ArrayOfBiomeGenBase[i] = ai[i] >= 0 ? BiomeGenBase.getBiome(ai[i]) : BiomeGenBase.taiga;
        }
        return par1ArrayOfBiomeGenBase;
    }
