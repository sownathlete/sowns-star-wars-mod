...
public class BiomeChunkProviderExegol
extends ChunkProviderGenerate {
    ...
    public BiomeChunkProviderExegol(World world, long seed, boolean features) {
    ...
    public void replaceBlocksForBiome(int par1, int par2, Block[] par3ArrayOfBlock, byte[] par4ArrayOfByte, BiomeGenBase[] par5ArrayOfBiomeGenBase) {
        ...
        double d0 = 0.03125;
        this.stoneNoise = this.noiseGen4.func_151599_a(this.stoneNoise, par1 * 16, par2 * 16, 16, 16, d0 * 2.0, d0 * 2.0, 1.0);

        // Fetch the registered blocks
        Block stone1 = BlockRegister.getRegisteredBlock("exegol_stone_1");
        Block stone2 = BlockRegister.getRegisteredBlock("exegol_stone_2");
        Block filler = BlockRegister.getRegisteredBlock("exegol_stone_1"); // The base filler block

        Random chunkRand = new Random(par1 * 341873128712L + par2 * 132897987541L); // Seed-based randomness per chunk

        for (int x = 0; x < 16; ++x) {
            for (int z = 0; z < 16; ++z) {
                BiomeGenBase biome = par5ArrayOfBiomeGenBase[z + x * 16];
                biome.genTerrainBlocks(this.worldObj, this.rand, par3ArrayOfBlock, par4ArrayOfByte, par1 * 16 + x, par2 * 16 + z, this.stoneNoise[z + x * 16]);

                // Determine the height at which the stone should generate
                int height = (int) (this.stoneNoise[z + x * 16] * 2.5 + 72.0);

                for (int y = 0; y < 256; ++y) {
                    int index = (x * 16 + z) * 256 + y;
                    Block currentBlock = par3ArrayOfBlock[index];

                    // Only replace existing terrain blocks, not air
                    if (currentBlock != Blocks.air) {
                        if (y < height - 2) {
                            par3ArrayOfBlock[index] = filler; // Deep underground = solid filler
                        } else if (y >= height - 3 && y <= height) {
                            // Randomly choose between stone1 and stone2 for the surface layer
                            par3ArrayOfBlock[index] = chunkRand.nextBoolean() ? stone1 : stone2;
                        }
                    }
                }
            }
