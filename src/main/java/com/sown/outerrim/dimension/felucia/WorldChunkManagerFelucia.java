...
public class WorldChunkManagerFelucia extends DimensionHelperChunkMgr {
    ...
    protected WorldChunkManagerFelucia() {
        this.biomesToSpawn.add(FeluciaBiomes.feluciaJungle);
    }

    public WorldChunkManagerFelucia(long seed) {
        this();
        GenLayer[] agenlayer = GenLayerFelucia.makeTheWorld(seed);
        this.unzoomedBiomes = agenlayer[0];
        this.zoomedBiomes = agenlayer[1];
    }

    public WorldChunkManagerFelucia(World world) {
        this(world.getSeed());
    }

    @Override
    public BiomeGenBase getBiome() {
        return FeluciaBiomes.feluciaJungle;
    }
    ...
    public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int z, int length, int width) {
        ...
        for (int i = 0; i < length * width; ++i) {
            par1ArrayOfBiomeGenBase[i] = arrayOfInts[i] >= 0 ? BiomeGenBase.getBiome(arrayOfInts[i]) : FeluciaBiomes.feluciaJungle;
        }
    ...
    public BiomeGenBase[] getBiomeGenAt(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int y, int width, int length, boolean cacheFlag) {
        ...
        for (int i = 0; i < width * length; ++i) {
            par1ArrayOfBiomeGenBase[i] = ai[i] >= 0 ? BiomeGenBase.getBiome(ai[i]) : FeluciaBiomes.feluciaJungle;
        }
        return par1ArrayOfBiomeGenBase;
    }
