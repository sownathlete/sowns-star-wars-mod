...
public class WorldChunkManagerGeonosis
extends DimensionHelperChunkMgr {
    ...
    protected WorldChunkManagerGeonosis() {
        this.biomesToSpawn.add(GeonosisBiomes.geonosisDesert);
    }

    public WorldChunkManagerGeonosis(long seed) {
        this();
        GenLayer[] agenlayer = GenLayerGeonosis.makeTheWorld(seed);
        this.unzoomedBiomes = agenlayer[0];
        this.zoomedBiomes = agenlayer[1];
    }

    public WorldChunkManagerGeonosis(World world) {
        this(world.getSeed());
    }

    @Override
    public BiomeGenBase getBiome() {
        return GeonosisBiomes.geonosisDesert;
    }
    ...
    public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int z, int length, int width) {
        ...
        for (int i = 0; i < length * width; ++i) {
            par1ArrayOfBiomeGenBase[i] = arrayOfInts[i] >= 0 ? BiomeGenBase.getBiome(arrayOfInts[i]) : GeonosisBiomes.geonosisDesert;
        }
    ...
    public BiomeGenBase[] getBiomeGenAt(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int y, int width, int length, boolean cacheFlag) {
        ...
        for (int i = 0; i < width * length; ++i) {
            par1ArrayOfBiomeGenBase[i] = ai[i] >= 0 ? BiomeGenBase.getBiome(ai[i]) : GeonosisBiomes.geonosisDesert;
        }
        return par1ArrayOfBiomeGenBase;
    }
