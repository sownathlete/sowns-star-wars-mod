...
public class WorldChunkManagerHoth
extends DimensionHelperChunkMgr {
    ...
    protected WorldChunkManagerHoth() {
        this.biomesToSpawn.add(HothProvider.hoth);
    }

    public WorldChunkManagerHoth(long seed) {
        this();
        GenLayer[] agenlayer = GenLayerBiomeHoth.makeTheWorld(seed);
        this.unzoomedBiomes = agenlayer[0];
        this.zoomedBiomes = agenlayer[1];
    }

    public WorldChunkManagerHoth(World world) {
        this(world.getSeed());
    }

    @Override
    public BiomeGenBase getBiome() {
        return HothProvider.hoth;
    }
    ...
    public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int z, int length, int width) {
        ...
        for (int i = 0; i < length * width; ++i) {
            par1ArrayOfBiomeGenBase[i] = arrayOfInts[i] >= 0 ? BiomeGenBase.getBiome(arrayOfInts[i]) : HothProvider.hoth;
        }
    ...
    public BiomeGenBase[] getBiomeGenAt(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int y, int width, int length, boolean cacheFlag) {
        ...
        for (int i = 0; i < width * length; ++i) {
            par1ArrayOfBiomeGenBase[i] = ai[i] >= 0 ? BiomeGenBase.getBiome(ai[i]) : HothProvider.hoth;
        }
        return par1ArrayOfBiomeGenBase;
    }
