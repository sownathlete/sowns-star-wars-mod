...
public class GenLayerIlum extends GenLayer {
    public GenLayerIlum(long seed) {
        super(seed);
    }
    ...
    public int[] getInts(int areaX, int areaY, int areaWidth, int areaHeight) {
        ...
        for (int y = 0; y < areaHeight; y++) {
            for (int x = 0; x < areaWidth; x++) {
                ...
                // 50/50 chance between the two Ilum biomes
                if (nextInt(2) == 0) {
                    biomes[x + y * areaWidth] = OuterRimResources.ConfigOptions.biomeIlumPlainsId;  // ilum biome
                } else {
                    biomes[x + y * areaWidth] = OuterRimResources.ConfigOptions.biomeIlumMountainsId;  // ilumPlateau biome
                }
            }
        }
    ...
    public static GenLayer[] makeTheWorld(long seed) {
        GenLayer biomes = new GenLayerIlumBiomes(1L);
