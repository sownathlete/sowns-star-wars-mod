...
public class WorldChunkManagerIlum extends DimensionHelperChunkMgr {
    ...
    protected WorldChunkManagerIlum() {
        this.biomesToSpawn.add(IlumBiomes.ilum);
    }

    public WorldChunkManagerIlum(long seed) {
        this();
        GenLayer[] agenlayer = GenLayerIlum.makeTheWorld(seed);
        this.unzoomedBiomes = agenlayer[0];
        this.zoomedBiomes = agenlayer[1];
    }

    public WorldChunkManagerIlum(World world) {
        this(world.getSeed());
    }

    @Override
    public BiomeGenBase getBiome() {
        return IlumBiomes.ilum;
    }
    ...
    public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int z, int length, int width) {
        ...
        for (int i = 0; i < length * width; ++i) {
            par1ArrayOfBiomeGenBase[i] = arrayOfInts[i] >= 0 ? BiomeGenBase.getBiome(arrayOfInts[i]) : IlumBiomes.ilum;
        }
    ...
    public BiomeGenBase[] getBiomeGenAt(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int y, int width, int length, boolean cacheFlag) {
        ...
        for (int i = 0; i < width * length; ++i) {
            par1ArrayOfBiomeGenBase[i] = ai[i] >= 0 ? BiomeGenBase.getBiome(ai[i]) : IlumBiomes.ilum;
        }
        return par1ArrayOfBiomeGenBase;
    }
