...
public class GenLayerBiomeKashyyyk extends GenLayer {

    public GenLayerBiomeKashyyyk(long seed) {
        super(seed);
    }
    ...
    public int[] getInts(int areaX, int areaY, int areaWidth, int areaHeight) {
        ...
        for (int y = 0; y < areaHeight; y++) {
            for (int x = 0; x < areaWidth; x++) {
                ...
                if (nextInt < 30) {
                    biomes[x + y * areaWidth] = BiomeGenBase.ocean.biomeID;
                } else if (nextInt < 60) {
                    biomes[x + y * areaWidth] = BiomeGenBase.deepOcean.biomeID;
                } else if (nextInt < 99) {
                    biomes[x + y * areaWidth] = KashyyykProvider.kashyyyk.biomeID;
                } else {
                    biomes[x + y * areaWidth] = KashyyykProvider.kashyyyk.biomeID;
                }
            }
    ...
    public static GenLayer[] makeTheWorld(long seed) {
        GenLayer biomes = new GenLayerKashyyykBiomes(1L);
