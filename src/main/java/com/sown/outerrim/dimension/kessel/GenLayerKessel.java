...
import com.sown.outerrim.dimension.naboo.GenLayerNabooBiomes;
...
public class GenLayerKessel extends GenLayer {
    public GenLayerKessel(long seed) {
        super(seed);
    }
        ...
        for (int y = 0; y < areaHeight; y++) {
            for (int x = 0; x < areaWidth; x++) {
                ...
                if (nextInt < 30) {  // 30% chance
                    biomes[x + y * areaWidth] = OuterRimResources.ConfigOptions.biomeKesselMountainsId;
                } else if (nextInt < 65) {  // 35% chance
                    biomes[x + y * areaWidth] = OuterRimResources.ConfigOptions.biomeKesselMountainsId;
                } else if (nextInt < 88) {  // 23% chance
                    biomes[x + y * areaWidth] = OuterRimResources.ConfigOptions.biomeKesselRock2Id;
                } else if (nextInt < 99) {  // 11% chance
                    biomes[x + y * areaWidth] = OuterRimResources.ConfigOptions.biomeKesselDirt1Id;
                } else {  // 1% chance
                    biomes[x + y * areaWidth] = OuterRimResources.ConfigOptions.biomeKesselMudId;
                }
            }
        }
    ...
    public static GenLayer[] makeTheWorld(long seed) {
        GenLayer biomes = new GenLayerKesselBiomes(1L);
