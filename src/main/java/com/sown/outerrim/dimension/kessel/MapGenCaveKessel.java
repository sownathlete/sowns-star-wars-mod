...
extends MapGenBaseMeta {
    ...
    protected void generateCaveNode(long par1, int par3, int par4, Block[] blockIdArray, byte[] metaArray, double par6, double par8, double par10, float par12, float par13, float par14, int par15, int par16, double par17) {
        ...
        boolean bl = flag1;
        while (par15 < par16) {
            double d6 = 1.5 + (double)(MathHelper.sin((float)((float)par15 * 3.1415927f / (float)par16)) * par12 * 1.0f);
            double d7 = d6 * par17;
            ...
            par14 = flag1 ? (par14 = par14 * 0.92f) : (par14 = par14 * 0.7f);
            par14 += f4 * 0.1f;
            par13 += f3 * 0.1f;
            f4 *= 0.9f;
            f3 *= 0.75f;
