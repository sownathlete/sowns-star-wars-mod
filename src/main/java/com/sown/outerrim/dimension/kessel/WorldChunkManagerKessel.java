...
import com.sown.outerrim.dimension.kessel.GenLayerKessel;
...
public class WorldChunkManagerKessel
extends DimensionHelperChunkMgr {
    ...
    protected WorldChunkManagerKessel() {
        this.biomesToSpawn.add(KesselBiomes.kesselRock1);
    }

    public WorldChunkManagerKessel(long seed) {
        this();
        GenLayer[] agenlayer = GenLayerKessel.makeTheWorld(seed);
        this.unzoomedBiomes = agenlayer[0];
        this.zoomedBiomes = agenlayer[1];
    }

    public WorldChunkManagerKessel(World world) {
        this(world.getSeed());
    }

    @Override
    public BiomeGenBase getBiome() {
        return KesselBiomes.kesselRock1;
    }
    ...
    public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int z, int length, int width) {
        ...
        for (int i = 0; i < length * width; ++i) {
            par1ArrayOfBiomeGenBase[i] = arrayOfInts[i] >= 0 ? BiomeGenBase.getBiome((int)arrayOfInts[i]) : KesselBiomes.kesselRock1;
        }
    ...
    public BiomeGenBase[] getBiomeGenAt(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int y, int width, int length, boolean cacheFlag) {
        ...
        for (int i = 0; i < width * length; ++i) {
            par1ArrayOfBiomeGenBase[i] = ai[i] >= 0 ? BiomeGenBase.getBiome((int)ai[i]) : KesselBiomes.kesselRock1;
        }
        return par1ArrayOfBiomeGenBase;
    }
    ...
    public boolean areBiomesViable(int par1, int par2, int par3, List par4List) {
        ...
        for (int k1 = 0; k1 < i1 * j1; ++k1) {
            ...
            if (par4List.contains((Object)biomegenbase)) continue;
            return false;
        }
        return true;
    }
    ...
    public ChunkPosition findBiomePosition(int par1, int par2, int par3, List par4List, Random par5Random) {
        ...
        for (int l1 = 0; l1 < ai.length; ++l1) {
            ...
            if (!par4List.contains((Object)biomegenbase) || chunkposition != null && par5Random.nextInt(k1 + 1) != 0) continue;
