...
public class BiomeGenKorribanMountains
extends OROverworldBiome {
    ...
    public BiomeGenKorribanMountains(int id) {
        ...
        if (Loader.isModLoaded("legends")) {
            ...
            this.topBlock = Blocks.sandstone; // Fallback to vanilla sandstone
        }
        if (Loader.isModLoaded("legends")) {
            this.fillerBlock = GameRegistry.findBlock("legends", "dynamicLegendsBlock1");
        } else {
            this.fillerBlock = Blocks.sandstone; // Fallback to vanilla sandstone
        }
        if (Loader.isModLoaded("legends")) {
            this.stoneBlock = GameRegistry.findBlock("legends", "dynamicLegendsBlock1");
        } else {
            this.stoneBlock = Blocks.sandstone; // Fallback to vanilla sandstone
        }
    ...
    public void generateBiomeTerrain(World world, Random rand, Block[] block, byte[] meta, int x, int z, double stoneNoise) {
        ...
        for (int y = 255; y >= 0; --y) {
            ...
            if (Loader.isModLoaded("legends")) {
                ...
                fillerBlock = Blocks.sandstone; // Fallback to vanilla sandstone
            }
            fillerMeta = 0;
        }
    }
                ...
                if (j1 == -1) {
                    ...
                    if (i1 <= 0) {
                        ...
                        if (Loader.isModLoaded("legends")) {
                            ...
                            block = Blocks.sandstone; // Fallback to vanilla sandstone
                        };
                        ...
                        if (l1 > 66 + i1) {
                            ...
                            if (b0 < 16) {
                                if (Loader.isModLoaded("legends")) {
                                	...
                                	par3ArrayOfBlock[i2] = Blocks.sandstone; // Fallback to vanilla sandstone
                                }
                    ...
                    if (block2 != Blocks.sand) continue;
                    par4ArrayOfByte[i2] = 1;
                    continue;
                }
                if (j1 <= 0) continue;
