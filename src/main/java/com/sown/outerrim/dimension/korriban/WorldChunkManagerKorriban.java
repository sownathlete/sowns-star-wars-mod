...
public class WorldChunkManagerKorriban extends DimensionHelperChunkMgr {
    ...
    protected WorldChunkManagerKorriban() {
        this.biomesToSpawn.add(KorribanBiomes.korribanDesert);
    }

    public WorldChunkManagerKorriban(long seed) {
        this();
        GenLayer[] agenlayer = GenLayerKorriban.makeTheWorld(seed);
        this.unzoomedBiomes = agenlayer[0];
        this.zoomedBiomes = agenlayer[1];
    }

    public WorldChunkManagerKorriban(World world) {
        this(world.getSeed());
    }

    @Override
    public BiomeGenBase getBiome() {
        return KorribanBiomes.korribanDesert;
    }
    ...
    public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int z, int length, int width) {
        ...
        for (int i = 0; i < length * width; ++i) {
            par1ArrayOfBiomeGenBase[i] = arrayOfInts[i] >= 0 ? BiomeGenBase.getBiome(arrayOfInts[i]) : KorribanBiomes.korribanDesert;
        }
    ...
    public BiomeGenBase[] getBiomeGenAt(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int y, int width, int length, boolean cacheFlag) {
        ...
        for (int i = 0; i < width * length; ++i) {
            par1ArrayOfBiomeGenBase[i] = ai[i] >= 0 ? BiomeGenBase.getBiome(ai[i]) : KorribanBiomes.korribanDesert;
        }
        return par1ArrayOfBiomeGenBase;
    }
