...
public class GenLayerMandalore extends GenLayer {
    public GenLayerMandalore(long seed) {
        super(seed);
    }
    ...
    public int[] getInts(int areaX, int areaY, int areaWidth, int areaHeight) {
        ...
        for (int y = 0; y < areaHeight; y++) {
            for (int x = 0; x < areaWidth; x++) {
                ...
                // 50/50 chance between the two Mandalore biomes
                if (nextInt(2) == 0) {
                    biomes[x + y * areaWidth] = OuterRimResources.ConfigOptions.biomeMandaloreId;  // mandalore biome
                } else {
                    biomes[x + y * areaWidth] = OuterRimResources.ConfigOptions.biomeMandaloreId;  // mandalorePlateau biome
                }
            }
        }
    ...
    public static GenLayer[] makeTheWorld(long seed) {
        GenLayer biomes = new GenLayerMandaloreBiomes(1L);
