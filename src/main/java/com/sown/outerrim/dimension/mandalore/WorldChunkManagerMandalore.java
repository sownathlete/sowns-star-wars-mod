...
public class WorldChunkManagerMandalore extends DimensionHelperChunkMgr {
    ...
    protected WorldChunkManagerMandalore() {
        this.biomesToSpawn.add(MandaloreBiomes.mandalore);
    }

    public WorldChunkManagerMandalore(long seed) {
        this();
        GenLayer[] agenlayer = GenLayerMandalore.makeTheWorld(seed);
        this.unzoomedBiomes = agenlayer[0];
        this.zoomedBiomes = agenlayer[1];
    }

    public WorldChunkManagerMandalore(World world) {
        this(world.getSeed());
    }

    @Override
    public BiomeGenBase getBiome() {
        return MandaloreBiomes.mandalore;
    }
    ...
    public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int z, int length, int width) {
        ...
        for (int i = 0; i < length * width; ++i) {
            par1ArrayOfBiomeGenBase[i] = arrayOfInts[i] >= 0 ? BiomeGenBase.getBiome(arrayOfInts[i]) : MandaloreBiomes.mandalore;
        }
    ...
    public BiomeGenBase[] getBiomeGenAt(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int y, int width, int length, boolean cacheFlag) {
        ...
        for (int i = 0; i < width * length; ++i) {
            par1ArrayOfBiomeGenBase[i] = ai[i] >= 0 ? BiomeGenBase.getBiome(ai[i]) : MandaloreBiomes.mandalore;
        }
        return par1ArrayOfBiomeGenBase;
    }
