...
public class WorldChunkManagerMustafar
extends DimensionHelperChunkMgr {
    ...
    protected WorldChunkManagerMustafar() {
        this.biomesToSpawn.add(MustafarBiomes.mustafarPlateau);
    }

    public WorldChunkManagerMustafar(long seed) {
        this();
        GenLayer[] agenlayer = GenLayerMustafar.makeTheWorld(seed);
        this.unzoomedBiomes = agenlayer[0];
        this.zoomedBiomes = agenlayer[1];
    }

    public WorldChunkManagerMustafar(World world) {
        this(world.getSeed());
    }

    @Override
    public BiomeGenBase getBiome() {
        return MustafarBiomes.mustafarPlateau;
    }
    ...
    public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int z, int length, int width) {
        ...
        for (int i = 0; i < length * width; ++i) {
            par1ArrayOfBiomeGenBase[i] = arrayOfInts[i] >= 0 ? BiomeGenBase.getBiome(arrayOfInts[i]) : MustafarBiomes.mustafarPlateau;
        }
    ...
    public BiomeGenBase[] getBiomeGenAt(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int y, int width, int length, boolean cacheFlag) {
        ...
        for (int i = 0; i < width * length; ++i) {
            par1ArrayOfBiomeGenBase[i] = ai[i] >= 0 ? BiomeGenBase.getBiome(ai[i]) : MustafarBiomes.mustafarPlateau;
        }
        return par1ArrayOfBiomeGenBase;
    }
