...
public class WorldChunkManagerNaboo
extends DimensionHelperChunkMgr {
    ...
    protected WorldChunkManagerNaboo() {
        this.biomesToSpawn.add(NabooProvider.nabooGreatPlains);
    }

    public WorldChunkManagerNaboo(long seed) {
        this();
        GenLayer[] agenlayer = GenLayerBiomeNaboo.makeTheWorld(seed);
        this.unzoomedBiomes = agenlayer[0];
        this.zoomedBiomes = agenlayer[1];
    }

    public WorldChunkManagerNaboo(World world) {
        this(world.getSeed());
    }

    @Override
    public BiomeGenBase getBiome() {
        return NabooProvider.nabooGreatPlains;
    }
    ...
    public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int z, int length, int width) {
        ...
        for (int i = 0; i < length * width; ++i) {
            par1ArrayOfBiomeGenBase[i] = arrayOfInts[i] >= 0 ? BiomeGenBase.getBiome(arrayOfInts[i]) : NabooProvider.nabooGreatPlains;
        }
    ...
    public BiomeGenBase[] getBiomeGenAt(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int y, int width, int length, boolean cacheFlag) {
        ...
        for (int i = 0; i < width * length; ++i) {
            par1ArrayOfBiomeGenBase[i] = ai[i] >= 0 ? BiomeGenBase.getBiome(ai[i]) : NabooProvider.nabooGreatPlains;
        }
        return par1ArrayOfBiomeGenBase;
    }
