...
public class BiomeChunkProviderNiamos
extends ChunkProviderGenerate {
    ...
    public BiomeChunkProviderNiamos(World world, long seed, boolean features) {
    ...
    public void replaceBlocksForBiome(int chunkX, int chunkZ,
            Block[] blocks, byte[] metadata, BiomeGenBase[] biomes)
    {
        ChunkProviderEvent.ReplaceBiomeBlocks event =
            new ChunkProviderEvent.ReplaceBiomeBlocks(
                this, chunkX, chunkZ,
                blocks, metadata, biomes,
                this.worldObj
            );
        ...
        this.stoneNoise = this.noiseGen4.func_151599_a(
            ...
            chunkX * 16, chunkZ * 16,
            16, 16,
            noiseScale * 2.0, noiseScale * 2.0, 1.0
        );
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                int idxXZ = z * 16 + x;
                BiomeGenBase biome = biomes[idxXZ];
                biome.genTerrainBlocks(
                    this.worldObj, this.rand,
                    blocks, metadata,
        ...
        final int SEA_LEVEL = 63;
        int patchCount = 1 + this.rand.nextInt(3);
        for (int pi = 0; pi < patchCount; pi++) {
            int cx = rand.nextInt(16), cz = rand.nextInt(16);

            int surfaceY = -1;
            for (int y = 255; y > SEA_LEVEL; y--) {
                int idx = (y << 8) | (cz << 4) | cx;
            ...
            int centerIdx = (surfaceY << 8) | (cz << 4) | cx;
            if (blocks[centerIdx] != Blocks.sand) continue;

            int radius = 3 + rand.nextInt(5);
            for (int dx = -radius; dx <= radius; dx++) {
                for (int dz = -radius; dz <= radius; dz++) {
                    double dist = Math.sqrt(dx*dx + dz*dz);
                    double fuzz = radius * (0.7 + rand.nextFloat()*0.6);
                    ...
                    int colY = -1;
                    for (int y = 255; y > SEA_LEVEL; y--) {
                        int idx2 = (y << 8) | (lz << 4) | lx;
                        if (blocks[idx2] != Blocks.air) {
                            colY = y;
                            break;
                        }
                    }
                    if (colY <= SEA_LEVEL) continue;

                    int idx2 = (colY << 8) | (lz << 4) | lx;
                    if (blocks[idx2] == Blocks.sand) {
                        blocks[idx2]   = Blocks.grass;
                        metadata[idx2] = 0;
                    }
                }
            }
