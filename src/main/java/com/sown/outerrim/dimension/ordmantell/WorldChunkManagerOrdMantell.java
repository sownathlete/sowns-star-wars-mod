...
public class WorldChunkManagerOrdMantell extends DimensionHelperChunkMgr {
	...
	protected WorldChunkManagerOrdMantell() {
		biomesToSpawn.add(BiomeGenBase.desertHills);
		biomesToSpawn.add(BiomeGenBase.mesaPlateau);
		biomesToSpawn.add(BiomeGenBase.savannaPlateau);
		biomesToSpawn.add(BiomeGenBase.plains);
		biomesToSpawn.add(BiomeGenBase.extremeHills);
	}

	public WorldChunkManagerOrdMantell(long seed) {
		this();
		GenLayer[] agenlayer = GenLayerBiomeOrdMantell.makeTheWorld(seed);
		this.unzoomedBiomes = agenlayer[0];
		this.zoomedBiomes = agenlayer[1];
	}

	public WorldChunkManagerOrdMantell(World world) {
		this(world.getSeed());
	}
	...
	public BiomeGenBase getBiomeGenAt(int x, int z) {
		...
		return biome != null ? biome : BiomeGenBase.mesaPlateau;
	}
	...
	public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] arr, int x, int z, int w, int h) {
		...
		for (int i = 0; i < w * h; ++i)
			arr[i] = ids[i] >= 0 ? BiomeGenBase.getBiome(ids[i]) : BiomeGenBase.mesaPlateau;
		return arr;
	}

	@Override
	public BiomeGenBase[] loadBlockGeneratorData(BiomeGenBase[] arr, int x, int z, int w, int h) {
		return getBiomeGenAt(arr, x, z, w, h, true);
	}
