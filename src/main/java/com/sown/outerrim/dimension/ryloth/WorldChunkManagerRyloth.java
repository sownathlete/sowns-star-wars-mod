...
public class WorldChunkManagerRyloth extends DimensionHelperChunkMgr {
    ...
    protected WorldChunkManagerRyloth() {
        this.biomesToSpawn.add(RylothBiomes.ryloth);
    }

    public WorldChunkManagerRyloth(long seed) {
        this();
        GenLayer[] agenlayer = GenLayerRyloth.makeTheWorld(seed);
        this.unzoomedBiomes = agenlayer[0];
        this.zoomedBiomes = agenlayer[1];
    }

    public WorldChunkManagerRyloth(World world) {
        this(world.getSeed());
    }

    @Override
    public BiomeGenBase getBiome() {
        return RylothBiomes.ryloth;
    }
    ...
    public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int z, int length, int width) {
        ...
        for (int i = 0; i < length * width; ++i) {
            par1ArrayOfBiomeGenBase[i] = arrayOfInts[i] >= 0 ? BiomeGenBase.getBiome(arrayOfInts[i]) : RylothBiomes.ryloth;
        }
    ...
    public BiomeGenBase[] getBiomeGenAt(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int y, int width, int length, boolean cacheFlag) {
        ...
        for (int i = 0; i < width * length; ++i) {
            par1ArrayOfBiomeGenBase[i] = ai[i] >= 0 ? BiomeGenBase.getBiome(ai[i]) : RylothBiomes.ryloth;
        }
        return par1ArrayOfBiomeGenBase;
    }
