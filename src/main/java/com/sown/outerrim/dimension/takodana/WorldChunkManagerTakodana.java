 ...
 */
package com.sown.outerrim.dimension.takodana;
...
public class WorldChunkManagerTakodana
extends DimensionHelperChunkMgr {
    ...
    protected WorldChunkManagerTakodana() {
        this.biomesToSpawn.add(BiomeGenBase.beach);
    }

    public WorldChunkManagerTakodana(long seed) {
        this();
        GenLayer[] agenlayer = GenLayerBiomeTakodana.makeTheWorld(seed);
        this.unzoomedBiomes = agenlayer[0];
        this.zoomedBiomes = agenlayer[1];
    }

    public WorldChunkManagerTakodana(World world) {
        this(world.getSeed());
    }
    ...
    public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int z, int length, int width) {
        ...
        for (int i = 0; i < length * width; ++i) {
            par1ArrayOfBiomeGenBase[i] = arrayOfInts[i] >= 0 ? BiomeGenBase.getBiome(arrayOfInts[i]) : BiomeGenBase.beach;
        }
    ...
    public BiomeGenBase[] getBiomeGenAt(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int y, int width, int length, boolean cacheFlag) {
        ...
        for (int i = 0; i < width * length; ++i) {
            par1ArrayOfBiomeGenBase[i] = ai[i] >= 0 ? BiomeGenBase.getBiome(ai[i]) : BiomeGenBase.beach;
        }
        return par1ArrayOfBiomeGenBase;
    }
