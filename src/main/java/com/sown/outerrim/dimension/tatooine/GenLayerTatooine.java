...
public class GenLayerTatooine extends GenLayer {
    public GenLayerTatooine(long seed) {
        super(seed);
    }
        ...
        for (int y = 0; y < areaHeight; y++) {
            for (int x = 0; x < areaWidth; x++) {
                ...
                if (nextInt < 30) {  // 30% chance
                    biomes[x + y * areaWidth] = OuterRimResources.ConfigOptions.biomeTatooineDesertId;
                } else if (nextInt < 65) {  // 35% chance
                    biomes[x + y * areaWidth] = OuterRimResources.ConfigOptions.biomeTatooineMountainsId;
                } else if (nextInt < 88) {  // 23% chance
                    biomes[x + y * areaWidth] = OuterRimResources.ConfigOptions.biomeTatooineDesertId;
                } else if (nextInt < 99) {  // 11% chance
                    biomes[x + y * areaWidth] = BiomeGenBase.mesa.biomeID;
                } else {  // 1% chance
                    biomes[x + y * areaWidth] = OuterRimResources.ConfigOptions.biomeTatooineDesertId;
                }
            }
    ...
    public static GenLayer[] makeTheWorld(long seed) {
        GenLayer biomes = new GenLayerTatooineBiomes(1L);
