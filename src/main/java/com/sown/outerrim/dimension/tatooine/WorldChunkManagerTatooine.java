...
public class WorldChunkManagerTatooine
extends DimensionHelperChunkMgr {
    ...
    protected WorldChunkManagerTatooine() {
        this.biomesToSpawn.add(TatooineBiomes.tatooineDesert);
    }

    public WorldChunkManagerTatooine(long seed) {
        this();
        GenLayer[] agenlayer = GenLayerTatooine.makeTheWorld(seed);
        this.unzoomedBiomes = agenlayer[0];
        this.zoomedBiomes = agenlayer[1];
    }

    public WorldChunkManagerTatooine(World world) {
        this(world.getSeed());
    }

    @Override
    public BiomeGenBase getBiome() {
        return TatooineBiomes.tatooineDesert;
    }
    ...
    public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int z, int length, int width) {
        ...
        for (int i = 0; i < length * width; ++i) {
            par1ArrayOfBiomeGenBase[i] = arrayOfInts[i] >= 0 ? BiomeGenBase.getBiome(arrayOfInts[i]) : TatooineBiomes.tatooineDesert;
        }
    ...
    public BiomeGenBase[] getBiomeGenAt(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int y, int width, int length, boolean cacheFlag) {
        ...
        for (int i = 0; i < width * length; ++i) {
            par1ArrayOfBiomeGenBase[i] = ai[i] >= 0 ? BiomeGenBase.getBiome(ai[i]) : TatooineBiomes.tatooineDesert;
        }
        return par1ArrayOfBiomeGenBase;
    }
