...
public class ChunkProviderTython extends ChunkProviderDryPreset {
	...
	public ChunkProviderTython(World world, long seed, boolean features) {
	...
	public void replaceBlocksForBiome(int par1, int par2, Block[] par3ArrayOfBlock, byte[] par4ArrayOfByte,
	    ...
	    ChunkProviderEvent.ReplaceBiomeBlocks event = new ChunkProviderEvent.ReplaceBiomeBlocks(
	            (IChunkProvider) this, par1, par2, par3ArrayOfBlock, par4ArrayOfByte, par5ArrayOfBiomeGenBase, this.worldObj);
	    ...
	    double d0 = 0.03125;
	    this.stoneNoise = this.noiseGen4.func_151599_a(this.stoneNoise, (double) (par1 * 16), (double) (par2 * 16), 
	            16, 16, d0 * 2.0, d0 * 2.0, 1.0);
	    ...
	    // Now, replace dirt blocks at y = 61 if the block above is air or tallgrass
	    for (int k = 0; k < 16; ++k) {
	        for (int l = 0; l < 16; ++l) {
	            int index = (k * 16 + l) * 256 + 61; // y = 61 index calculation
	            int indexAbove = index + 1; // Block above (y = 62)

	            if (par3ArrayOfBlock[index] == Blocks.dirt &&
	               (par3ArrayOfBlock[indexAbove] == Blocks.air || par3ArrayOfBlock[indexAbove] == Blocks.tallgrass)) {
	                par3ArrayOfBlock[index] = Blocks.grass;
	            }
	        }
	    }
	}
