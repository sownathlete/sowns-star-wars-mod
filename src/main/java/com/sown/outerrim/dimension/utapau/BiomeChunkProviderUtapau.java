...
public class BiomeChunkProviderUtapau
extends ChunkProviderGenerate {
    ...
    public BiomeChunkProviderUtapau(World world, long seed, boolean features) {
    ...
    public Chunk provideChunk(int chunkX, int chunkZ) {
        ...
        // Replace stone with sandstone
        for (int i = 0; i < blocks.length; i++) {
            if (blocks[i] == Blocks.stone) {
                blocks[i] = Blocks.sandstone;
            }
        }

        // Force set Y=4 to Y=9 layers to a 50/50 mix of sandstone and stone
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                for (int y = 4; y <= 9; y++) {
                    int index = (x * 16 + z) * 256 + y;
                    blocks[index] = (this.rand.nextBoolean()) ? Blocks.stone : Blocks.sandstone;
                }
            }
        }

        // NEW: 2% chance to turn sandstone into stone for Y >= 10
        for (int i = 0; i < blocks.length; i++) {
            int y = i & 255; // index % 256
            if (y >= 10 && blocks[i] == Blocks.sandstone && this.rand.nextInt(100) < 10) {
