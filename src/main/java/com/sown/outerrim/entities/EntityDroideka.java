...
public class EntityDroideka extends EntityTameable implements IRangedAttackMob {
	...
	protected void applyEntityAttributes() {
		...
		this.getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(40.0); // Logical health
		this.getEntityAttribute(SharedMonsterAttributes.movementSpeed).setBaseValue(0.01); // Reasonable speed
	}
	...
	public void attackEntityWithRangedAttack(EntityLivingBase target, float distanceFactor) {
		...
		// Adjust the position to make sure the projectile originates from the entity's
		// eye level
	...
	public boolean attackEntityFrom(DamageSource source, float amount) {
	    if (source.isProjectile() || source.isExplosion()) {
	        return false; // Immune to all projectiles and explosions
	    }
	...
	protected String getLivingSound() {
		return "outerrim:entity.droideka.step"; // Use ambient sound pool for when the droid is alive
	}

	@Override
	protected String getHurtSound() {
		return "outerrim:entity.droideka.step"; // Use hurt sound pool for when the droid gets hurt
	}

	@Override
	protected String getDeathSound() {
		return "outerrim:entity.droideka.step"; // Use death sound pool for when the droid dies
	}

	public void playMeleeSound() {
		this.playSound("outerrim:entity.droideka.step", 1.0F, 1.0F); // Play melee attack sound pool
	}

	public void playShootSound() {
		this.playSound("outerrim:entity.droideka.shoot", 1.0F, 1.0F); // Sound event when the clone trooper shoots
	}

	public void playPanicSound() {
		this.playSound("outerrim:entity.droideka.step", 1.0F, 1.0F); // Play panic sound pool
	}
    ...
    public void onUpdate() {
        super.onUpdate();
        this.isShieldActive = true; // Ensure shield stays on
    }
