...
/**
 * b1_battle_droid - Undefined Created using Tabula 4.1.1
 */
public class ModelBattleDroid extends ModelBase {
	...
	public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw,
		...
		this.arm_right.rotateAngleX = this.arm_right.rotateAngleY = this.arm_right.rotateAngleZ = 0F;
		this.arm_left.rotateAngleX = this.arm_left.rotateAngleY = this.arm_left.rotateAngleZ = 0F;

		this.leg_right.rotateAngleX = this.leg_right.rotateAngleY = this.leg_right.rotateAngleZ = 0F;
		this.leg_left.rotateAngleX = this.leg_left.rotateAngleY = this.leg_left.rotateAngleZ = 0F;

		this.calf_right.rotateAngleX = this.calf_right.rotateAngleY = this.calf_right.rotateAngleZ = 0F;
		this.calf_left.rotateAngleX = this.calf_left.rotateAngleY = this.calf_left.rotateAngleZ = 0F;

		// Forearms default to the constructor pose (-90°). Do NOT lerp from current.
		this.forearm_right.rotateAngleX = -(float) Math.PI / 2F;
		this.forearm_right.rotateAngleY = 0F;
		this.forearm_right.rotateAngleZ = 0F;

		this.forearm_left.rotateAngleX = -(float) Math.PI / 2F;
		this.forearm_left.rotateAngleY = 0F;
		this.forearm_left.rotateAngleZ = 0F;

		// ---- Head: base tilt + input yaw/pitch (purely from params) ----
		final float RAD = (float) Math.PI / 180F;
		this.head.rotateAngleY = netHeadYaw * RAD;
		this.head.rotateAngleX = 0.5672320069F + headPitch * RAD; // keeps your base tilt

		// ---- Walk cycle (deterministic; no cross-entity state) ----
		final float walkFreq = 0.6662F;
		float swingR = MathHelper.cos(limbSwing * walkFreq) * limbSwingAmount;
		float swingL = MathHelper.cos(limbSwing * walkFreq + (float) Math.PI) * limbSwingAmount;

		// Legs oppose
		this.leg_right.rotateAngleX = swingR;
		this.leg_left.rotateAngleX = swingL;

		// Calves slight counterbend
		this.calf_right.rotateAngleX = -0.2F * swingR;
		this.calf_left.rotateAngleX = -0.2F * swingL;

		// Arms mild counter to legs
		this.arm_right.rotateAngleX = -0.4F * swingR;
		this.arm_left.rotateAngleX = -0.4F * swingL;

		// Optional tiny idle jitter that’s per-entity (uses ageInTicks which is
		// entity-local)
		float idle = (1.0F - clamp01(limbSwingAmount * 3.0F)) * 0.05F * MathHelper.sin(ageInTicks * 0.07F);
		this.arm_right.rotateAngleZ += idle;
		this.arm_left.rotateAngleZ -= idle;
	}
