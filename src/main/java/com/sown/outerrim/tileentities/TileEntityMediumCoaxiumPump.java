...
public class TileEntityMediumCoaxiumPump extends TileEntity implements IInventory {
	private static final int SLOTS = 9;
	private static final String SOURCE_NAME = "outerrim:coaxiumDepositMedium";
	...
	public final int[] clientProgress = new int[SLOTS];

	private void dbg(String msg) {
		System.out.println("[Pump-Medium] (" + xCoord + "," + yCoord + "," + zCoord + ") " + msg);
	}
		...
		for (int i = 0; i < SLOTS; i++) {
			...
			if (hasEmpty && valid) {
				...
				if (progress[i] >= PROCESS_TIME) {
					ItemStack out = new ItemStack(ItemRegister.getRegisteredItem("vialCoaxiumRefined"));
					NBTTagCompound tag = new NBTTagCompound();
					tag.setInteger("Volatility", 2);
