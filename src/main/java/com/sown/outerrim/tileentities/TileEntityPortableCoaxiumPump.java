...
public class TileEntityPortableCoaxiumPump extends TileEntity implements IInventory {
	...
	public void updateEntity() {
		...
		boolean hasEmpty = slot != null && slot.getItem() == ItemRegister.getRegisteredItem("vialEmpty");
		...
		if (hasEmpty && validOre) {
			...
			if (progress >= PROCESS_TIME) {
				...
				int volatility = 30 + worldObj.rand.nextInt(26);
	...
	public int getProgressScaled(int px) {
		return clientProgress * px / PROCESS_TIME;
	}
	...
	public void readFromNBT(NBTTagCompound n) {
		...
		slot = n.hasKey("Slot") ? ItemStack.loadItemStackFromNBT(n.getCompoundTag("Slot")) : null;
	}
